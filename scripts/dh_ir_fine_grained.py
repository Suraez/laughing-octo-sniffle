...
BATCH_SIZE_IR = 4
BATCH_SIZE_DH = 4
MEMORY_HIGH = 104857600     # 100 MB
CGROUP_PARENT = "docker_elastic.slice"
...
ACTIVATION_LOG = f"activations_fine_grained_{datetime.now().strftime('%b%d').lower()}.log"
...
def set_memory_high_for_dh(mem_high: int, cgroup_parent: str):
    try:
        ...
        if not ids:
            print("[INFO] No _dh containers found when setting memory.high.")
            return
        ...
        for cid in ids:
            ...
            if not os.path.exists(scope):
                print(f"[WARN] Scope not found for container {cid}: {scope}")
                continue

            # 🟢 Terminal-only message (not sent to file)
            print(f"[INFO] Setting memory.high = {mem_high} for DH container {cid}")
        ...
        print(f"[ERROR] set_memory_high_for_dh: {e}")



def invoke_action_sync(action_url: str, batch_num: int, index: int):
    """Invoke a web action via HTTP POST and log only errors."""
...
def run_dh_batch(batch_num):
    ...
    set_memory_high_for_dh(MEMORY_HIGH, CGROUP_PARENT)
    return results
